...
- 改造现有产品入口的交互路径、跨组件状态流或协议／持久化边界时，正式实现前基于当前代码追踪用户入口、状态写入与回显，在对应 Issue／PR 记录已确认的行为依据和本次验收、实际复用／替换路径与状态归属、最终集成版本的可观察证据计划及未覆盖范围。长期行为或结构变化仍分别维护在 `docs/specs/`、`docs/architecture/`；动态计划和进度留在 Issue／PR，不为每个 FR 新建文档。必要语义与已接受决定冲突或尚未确定时，暂停受影响的实现并提出待决点；不受影响的工作及不跨上述边界的局部修改按现有规则直接继续。
