...
2. 同 Task 从正式 Composer 发送 `M2_RC_5B2_NAV_RUNNING_926` 长回答，在原执行仍 running 时进入[原自动化页](navigate-away-running.jpg)，随即从原侧栏回[原 Task](return-during-execution.jpg)；返回时“请求中断”仍在，原答复在原消息流继续增量。最终 `END_NAV_5B2_926` 在[同页完成态](return-after-complete.jpg)出现。产品 Input／Execution 归属原 Task／Session，原生 `session_input` 只有对应一条；33 个 `message.delta` 的首条早于 `execution.completed`，见[双库核对](crosscheck.json)。没有因导航重新发送输入。
