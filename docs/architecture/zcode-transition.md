...
固定提交列出 6,973 个路径；导入其中 6,717 个文件及符号链接。排除的 256 个路径为 `.agents/`、`.vscode/`、根 `.env*`、根 `AGENTS.md`、`README.md`、`README.en.md`、`CONTEXT.md`、`DESIGN.md`、`LICENSE`。排除仓库专用说明与环境配置，保留 AnyAgent 已有的规则、愿景、README 和 Apache-2.0 许可证。导入范围的完整文件清单由本仓库 Git tree 承载；上游版本及排除规则由本节记录。导入了上游 `NOTICE.md`、`THIRD-PARTY-NOTICES.md` 和 `third-party/`，其第三方条款仍需在任何分发前按实际产物核对。
