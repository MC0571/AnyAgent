...
| TanStack AI/AG-UI：继续候选评估，当前不引入 | TanStack AI 的评估范围包括具体 Engine Adapter 内的 Harness 接入辅助／事件转换、独立模型调用及 UI 辅助；AG-UI 的位置见 [ARC-05](README.md#ARC-05)；均须核查固定版本源码与契约，不据此宣称支持 | 与不使用该库的契约路径比较，Session、审批、取消、错误与原生扩展无隐式丢失，模型调用不冒充完整 Harness | 替换限制在具体 Adapter、转换或组装边界，不改领域身份、状态归属、公共行为与产品持久化格式 |
