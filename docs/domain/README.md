...
Session 是否可以接纳业务由 [LIFE-ADMISSION](lifecycle-and-ownership.md#LIFE-ADMISSION) 决定。按当前 Task 作用域提案，终态后的新目标创建关联的新 Task、新参与者和新 Session；旧 Session 不重绑到新 Task，原生恢复也不改变其历史归属。
...
Session 跨 Task 连续性仍待选择，不将当前 Task 作用域提案视为批准结果。跨 Task 复用 Session 的备选若被选择，需要另行定义任务授权隔离、旧上下文披露、并发归属及历史验收边界，不能只改一个关联字段。维护者可用以下三种场景比较两种方案：

| 连续性场景 | 当前提案的结果 | 跨 Task 复用方案需要回答的问题 |
| --- | --- | --- |
| 同 Task 内多轮实现、评审与修复 | 按参与者退回／重新分工及任务状态转换继续原 Session，每轮重新核对执行资格 | 不涉及跨 Task，无需为多轮工作改绑身份 |
| Task 已验收，用户沿用同一 Engine 做后续目标 | 新 Task／参与者／Session，经授权引用旧目标、摘要和产物 | 是否允许沿用原生上下文，旧资料授权及新验收责任如何分隔 |
| 常驻 Agent 连续处理多个独立 Task | 复用 Engine 与角色配置，每个 Task 新建参与者及 Session | 如何区分跨任务常驻身份与各任务职责／授权，原生上下文如何按新任务披露范围隔离 |
