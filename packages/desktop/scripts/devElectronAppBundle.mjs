...
export const DEV_ELECTRON_PROTOCOL_SCHEME = "anyagent";
export const DEV_ELECTRON_APP_NAME = "AnyAgent Dev";
export const DEV_ELECTRON_APP_BUNDLE_ID = "dev.anyagent.app.development";
...
export function patchDevElectronInfoPlist(plist) {
  ...
  patched = replacePlistString(patched, "CFBundleIconFile", "anyagent.icns");
...
export async function prepareDevElectronAppBundle({
  ...
  if (needsCopy) {
    ...
    await cp(join(import.meta.dirname, "../build/icon.icns"), join(appPath, "Contents/Resources/anyagent.icns"));
