...
case "${1:-}" in
  before)
    ...
    ;;
  after)
    [[ -f "$marker" && -f "$db" && -f "$log" ]] || { echo "Run 'bash scripts/m0-smoke.sh before' before the first launch and complete a logged chat." >&2; exit 1; }
    ...
    result="$(sqlite3 -readonly "$db" "SELECT EXISTS(SELECT 1 FROM message u JOIN part up ON up.message_id=u.id JOIN message a ON a.session_id=u.session_id AND a.sequence=u.sequence+1 JOIN part ap ON ap.message_id=a.id WHERE json_extract(u.data,'$.role')='user' AND json_extract(up.data,'$.text')='Reply exactly M0_GO_OK_9243' AND json_extract(a.data,'$.role')='assistant' AND json_extract(ap.data,'$.text')='M0_GO_OK_9243');")"
