...
任务历史记录用户做过什么、Agent 产生了什么结果以及哪些决定需要继续。Automation 与 Jobs 让重复工作可被安排和观察；共享 context 与 memory 让授权的经验和资料能够跨任务复用。
...
Task 与 Workflow 是跨 Engine 的协作边界，可以协调多个由相同或不同 Engine 驱动的 Agent Instance 及其 Session。Agent Instance 表达参与者的身份、所使用的 Engine 与承担的职责；每个 Session 始终绑定一个 Engine，承载对应 Engine 的持续对话与运行状态。

产品层通过共同契约协调跨 Session 的任务依赖、消息、产物、参与者状态、生命周期与权限。Agent 通过经授权的工作区、产物和显式消息协作，各自保留 Harness 的私有上下文、压缩机制、原生工具与运行状态。共享资产的访问、并发修改、冲突处理和结果整合应具有明确的归属与协调责任。

协作可以采用交接、串行、并行或交互式形式，并根据参与 Engine 的实际能力组合使用。统一契约不假定所有 Engine 都具备相同的消息、运行中介入或恢复能力，也不绕过 Engine 与执行宿主的权限边界。产品为用户提供观察、介入和中止协作的入口，如实呈现这些控制的生效状态与影响范围，并使各参与者的责任、信息来源和结果可以追踪。

### Session 恢复与 Handoff

恢复 Session 时继续使用原 Engine，使历史、上下文和原生工具状态的归属保持清楚。

Handoff 是跨 Engine 协作的一种交接形式：创建新会话，并传递目标、摘要、差异、文件、TODO 和必要上下文。交接清楚标明可复用的材料与无法迁移的私有状态；新 Engine 根据自己的能力重新建立上下文，用户能够追踪工作如何延续。

## 我们希望带来的改变

- 用户可以在同一工作环境中持续管理项目、任务、历史、浏览器和权限，Engine 变化不会迫使工作资产搬家。
- 用户可以根据任务选择或组合不同 Engine 驱动的 Agent，并清楚看到各参与者的职责、可用能力、授权范围以及协作控制的生效状态。
